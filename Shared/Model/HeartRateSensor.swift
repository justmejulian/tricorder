...
struct HeartRateSensor: Sensor {
    ...
    init(from decoder: any Decoder) throws {
        ...
        if self.name != name {
            throw DecodingError.dataCorruptedError(forKey: .name, in: try decoder.container(keyedBy: CodingKeys.self), debugDescription: "Expected name \(self.name) but found \(name).")
        }
